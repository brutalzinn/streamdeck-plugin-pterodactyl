...
class StreamDeck extends Api {
	 ...
	 * @param [target]
	 */
	static setTitle(context, title = '', target = Constants.hardwareAndSoftware) {
		if (!context) {
	 ...
	 * @param {number} [target]
	 */
	static clearTitle(context, target) {
		if (!context) {
	 ...
	 * @param {object} payload
	 */
	static sendToPropertyInspector(context, action, payload = null) {
	 ...
	 * @param {number} [target]
	 */
	static setImage(context, image, state, target = Constants.hardwareAndSoftware) {
		if (!context) {
...
function connectElgatoStreamDeckSocket(port, uuid, messageType, appInfoString, actionInfo) {
	StreamDeck.connect(port, uuid, messageType, appInfoString, actionInfo);
}
